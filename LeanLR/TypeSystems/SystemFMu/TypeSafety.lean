...
/-!
# System F with recursive types: syntactic type safety

The rest of `systemf_mu/types.v`: context inclusion and type substitution on contexts, the
...
theorem typed_weakening {n m : Nat} {Γ Δ : TypingContext} {e : Expr} {A : Ty}
  ...
  | typed_roll n Γ e A _ ih => exact .typed_roll m Δ e A (ih hsub hle)
  | typed_unroll n Γ e A _ ih => exact .typed_unroll m Δ e A (ih hsub hle)
...
theorem syn_typed_wf {n : Nat} {Γ : TypingContext} {e : Expr} {A : Ty}
  ...
  | typed_roll n Γ e A _ ih => exact .mu_wf (type_wf_rec_type n A (ih hΓ))
...
theorem typed_unroll' {n : Nat} {Γ : TypingContext} {e : Expr} {A B : Ty}
    (ht : SynTyped n Γ e (.mu A)) (heq : B = Ty.subst1 A (.mu A)) :
    SynTyped n Γ (.unroll e) B :=
...
theorem roll_inversion {n : Nat} {Γ : TypingContext} {e : Expr} {B : Ty}
    (h : SynTyped n Γ (.roll e) B) :
    ∃ A, B = .mu A ∧ SynTyped n Γ e (Ty.subst1 A (.mu A)) := by
  cases h
  exact ⟨_, rfl, by assumption⟩

theorem unroll_inversion {n : Nat} {Γ : TypingContext} {e : Expr} {B : Ty}
    (h : SynTyped n Γ (.unroll e) B) :
    ∃ A, B = Ty.subst1 A (.mu A) ∧ SynTyped n Γ e (.mu A) := by
  cases h
  exact ⟨_, rfl, by assumption⟩
...
theorem canonical_values_rec {n : Nat} {Γ : TypingContext} {e : Expr} {A : Ty}
    (ht : SynTyped n Γ e (.mu A)) (hv : Expr.isVal e) :
    ∃ e', e = .roll e' ∧ Expr.isVal e' := by
  suffices h : ∀ T : Ty, SynTyped n Γ e T → T = .mu A →
...
theorem typed_progress {e : Expr} {A : Ty}
  ...
  | typed_roll n Γ e A ht' ih =>
    ...
    · exact Or.inl hv
    · obtain ⟨e', hstep⟩ := hred
      exact Or.inr ⟨_, contextual_step_roll hstep⟩
  | typed_unroll n Γ e A ht' ih =>
    intro hΓ
    rcases ih hΓ with hv | hred
    · obtain ⟨e'', rfl, hv''⟩ := canonical_values_rec ht' hv
      exact Or.inr ⟨_, base_contextual_step (.unrollS e'' hv'')⟩
    · obtain ⟨e', hstep⟩ := hred
      exact Or.inr ⟨_, contextual_step_unroll hstep⟩
...
theorem fill_item_typing_decompose {Ki : EctxItem} {e : Expr} {A : Ty}
  ...
  case rollCtx =>
    ...
    exact ⟨Ty.subst1 A₂ (.mu A₂), hty, fun e' h' => .typed_roll 0 _ e' A₂ h'⟩
  case unrollCtx =>
    obtain ⟨A₂, rfl, hty⟩ := unroll_inversion h
    exact ⟨.mu A₂, hty, fun e' h' => .typed_unroll 0 _ e' A₂ h'⟩
